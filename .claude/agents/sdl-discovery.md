...
description: Scan repositories and generate draft SDL specifications from observable architecture evidence. Includes complexity scoring (v1.0) for architectural and operational assessment.
version: 1.2.0
---
...
- **Manifests:** package.json, pom.xml, Cargo.toml, go.mod, .csproj, build.gradle, etc.
- **Infrastructure:** Dockerfile, docker-compose.yml, Helm charts, K8s YAML, Terraform, CloudFormation
- **CI/CD:** GitHub Actions, Azure DevOps, GitLab CI, Jenkinsfiles
- **API Specs:** OpenAPI/Swagger, GraphQL, protobuf, AsyncAPI, Postman
- **Runtime:** Database migrations, connection code, auth setup, messaging configuration
- **Documentation:** README, architecture docs, ADRs, deployment guides
...
Infer:
- Service-to-service dependencies (HTTP calls, queue subscriptions, database sharing)
- Shared infrastructure (databases, caches, queues)
- Interface exposure (public vs. internal endpoints)
- Environment patterns (dev, staging, prod)
- Deployment grouping (which services are deployed together)
...
Produce a draft SDL v1.1 document that includes:
- `solution` section (name, description, stage)
- `architecture` section (style, projects/services)
- `data` section (datastores, connections)
- `auth` section (strategy, provider, token type)
- `integrations` section (external services)
- `deployment` section (cloud, CI/CD, targets)
- `contracts` section (APIs)
- `artifacts` section (docker images, deployment targets)
- `assumptions` section (key inferred relationships)
- Additional sections as applicable (observability, resilience, etc.)
