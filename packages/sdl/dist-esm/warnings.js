...
export function detectWarnings(sdl) {
    const warnings = [];
...
function checkComplexityVsTeam(sdl) {
    if (sdl.architecture.style !== 'microservices')
        return [];
    const team = sdl.constraints?.team;
    if (!team)
        return [];
    const totalDevs = team.developers ?? ((team.backend ?? 0) + (team.frontend ?? 0) + (team.fullstack ?? 0));
...
function checkTimelineVsScope(sdl) {
...
function checkAuthForMultiPersona(sdl) {
    if (sdl.auth)
        return [];
    const personas = sdl.product?.personas ?? [];
    if (personas.length <= 1)
        return [];
    const hasAdmin = personas.some(p => p.accessLevel === 'admin' ||
...
function checkBudgetVsInfrastructure(sdl) {
