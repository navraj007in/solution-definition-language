 ...
 */
export function generateADRs(doc) {
    const adrs = [];
    let counter = 1;
    // 1. Architecture pattern — always
...
// ─── ADR Builders ───
function buildArchitecturePatternADR(doc, n) {
...
function buildCloudHostingADR(doc, n) {
...
function buildAuthenticationADR(doc, n) {
    const auth = doc.auth;
...
function buildDatabaseADR(doc, n) {
...
function buildORMADR(doc, backendsWithORM, n) {
...
function buildFrontendFrameworkADR(doc, fe, n) {
...
function buildBackendFrameworkADR(doc, be, n) {
...
function architectureRationale(style, doc) {
...
function architectureConsequences(style) {
    switch (style) {
        case 'modular-monolith':
...
function cloudConsequences(cloud) {
    switch (cloud) {
        case 'aws':
...
function frameworkConsequences(fw) {
    switch (fw) {
        case 'nextjs':
...
function backendConsequences(fw) {
    switch (fw) {
        case 'nodejs':
