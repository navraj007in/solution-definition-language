 ...
 */
export function generateRepoScaffold(doc) {
    const files = [];
    ...
    return {
        ...
        metadata: {
            ...
            projectCount: (doc.architecture.projects.frontend || []).length +
...
// ─── Frontend Scaffolds ───
function generateFrontendScaffold(fe, doc) {
    ...
    if (doc.auth?.provider === 'auth0')
        deps['@auth0/nextjs-auth0'] = '3.5.0';
    if (doc.auth?.provider === 'clerk')
        deps['@clerk/nextjs'] = '5.0.0';
    if (doc.auth?.provider === 'supabase')
        deps['@supabase/supabase-js'] = '2.43.0';
    if (doc.auth?.provider === 'firebase')
...
// ─── Backend Scaffolds ───
function generateBackendScaffold(be, doc) {
...
// ─── Mobile Scaffolds ───
function generateMobileScaffold(mob, doc) {
...
function envExample(doc, layer) {
    ...
    if (layer === 'backend') {
        ...
        if (db.type === 'postgres')
            ...
            lines.push('DATABASE_URL=mysql://user:password@localhost:3306/db');
        else if (db.type === 'mongodb')
            lines.push('DATABASE_URL=mongodb://localhost:27017/db');
        else
...
function readme(projectName, framework, solutionName) {
    return `# ${projectName}
...
function nodeServerStub(doc, projectName) {
...
function pluralize(s) {
    if (s.endsWith('y') && !/[aeiou]y$/.test(s))
...
function envVarPrefix(fe) {
    if (fe.framework === 'nextjs')
        return 'NEXT_PUBLIC_';
    if (fe.framework === 'angular')
        return 'NG_APP_';
    return 'VITE_'; // react, vue, svelte, solid
}
function backendPort(framework) {
    switch (framework) {
...
function envFrontendPerEnv(doc, fe, backends, env) {
...
function envUrl(serviceName, port, env) {
    switch (env) {
...
function appUrl(projectName, env) {
    switch (env) {
...
function frontendConfigModule(fe, backends) {
    const prefix = envVarPrefix(fe);
...
function envBackendPerEnv(doc, be, otherBackends, services, env) {
    ...
    if (integrations) {
        ...
        if (hasAny) {
            ...
            if (integrations.monitoring?.provider) {
                ...
                else if (mon === 'newrelic')
                    lines.push('NEW_RELIC_LICENSE_KEY=');
            }
            lines.push('');
        }
    }
...
function backendConfigModule(doc, _be, otherBackends, services) {
    const lines = [];
    lines.push(`// Auto-generated environment config — edit .env.* files to change values`);
    lines.push('');
    ...
    if (allServices.length > 0) {
        ...
        for (const svc of allServices) {
            const camel = svc.name.replace(/[- ]([a-z])/g, (_, c) => c.toUpperCase());
