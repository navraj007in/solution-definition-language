...
function withTier(raw, tier) {
    if (tier === 'advisory') {
        const files = raw.files.map((f) => f.path.endsWith('.md') || f.path.endsWith('.mdc')
...
export function generateAll(doc) {
    const results = [];
    const skipped = [];
...
export function generateCiCd(doc) {
...
export function generateKubernetes(doc) {
...
export function generateCodingRules(doc) {
...
export function generateCodingRulesEnforcement(doc) {
...
export function generateComplianceChecklist(doc) {
...
export function summarizeGenerationResults(results) {
    ...
    const lines = [`Generated ${total} artifact${total !== 1 ? 's' : ''}:`];
    const tierLines = [];
