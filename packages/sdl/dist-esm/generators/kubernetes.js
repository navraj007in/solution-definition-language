...
function getBackendReplicas(doc) {
...
function renderConfigMap(ns, doc) {
    const lines = [];
...
function renderDeployment(ns, name, tier, port, replicas, framework) {
...
function renderService(ns, name, port) {
    const appLabel = `${ns}-${name}`;
    return `apiVersion: v1
kind: Service
metadata:
...
function renderHPA(ns, name, minReplicas) {
...
function renderIngress(ns, frontends, backends, doc) {
    const lines = [];
