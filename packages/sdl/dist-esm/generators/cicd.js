 ...
 */
export function generateCiCdPipeline(doc) {
    ...
    if (iacFiles) {
        ...
        cicdResult.metadata.hasIaC = true;
        cicdResult.metadata.iac = iac;
        // Backwards-compatible alias
        if (iac === 'terraform') {
            cicdResult.metadata.hasTerraform = true;
        }
    }
    ...
    if (dockerfiles.length > 0) {
        ...
        cicdResult.metadata.hasDockerfiles = true;
    }
    return cicdResult;
}
function generateGitHubActions(doc) {
...
function sanitizeJobId(s) {
    return s.replace(/[^a-zA-Z0-9_-]/g, '_');
}
function deployTargetLabel(cloud) {
    const labels = {
        aws: 'AWS',
        gcp: 'Google Cloud',
        azure: 'Azure',
...
function generateAzureDevOps(doc) {
    const frontends = doc.architecture.projects.frontend || [];
    const backends = doc.architecture.projects.backend || [];
    const dbType = doc.data.primaryDatabase.type;
    const lines = [];
...
function appendAzureDevOpsService(lines, dbType) {
    if (dbType === 'postgres') {
        lines.push('          postgres:');
...
function appendGitLabServices(lines, dbType) {
    if (dbType === 'postgres') {
        lines.push('  services:');
...
function generateDockerfiles(doc) {
    const files = [];
    const backends = doc.architecture.projects.backend || [];
    for (const be of backends) {
        const name = slugify(be.name);
...
function generateBicep(doc) {
    if (doc.deployment.cloud !== 'azure')
        return null;
    const backends = doc.architecture.projects.backend || [];
    const dbType = doc.data.primaryDatabase.type;
    const lines = [];
...
function generatePulumiAws(doc) {
    const backends = doc.architecture.projects.backend || [];
    const dbType = doc.data.primaryDatabase.type;
    const lines = [];
...
function generatePulumiGcp(doc) {
    const backends = doc.architecture.projects.backend || [];
    const dbType = doc.data.primaryDatabase.type;
    const lines = [];
    lines.push('import * as pulumi from "@pulumi/pulumi";');
...
function generatePulumiAzure(doc) {
    const backends = doc.architecture.projects.backend || [];
    const dbType = doc.data.primaryDatabase.type;
    const lines = [];
    lines.push('import * as pulumi from "@pulumi/pulumi";');
...
function generateCdk(doc) {
    if (doc.deployment.cloud !== 'aws')
        return null;
    const backends = doc.architecture.projects.backend || [];
    const dbType = doc.data.primaryDatabase.type;
...
function generateCloudFormation(doc) {
    if (doc.deployment.cloud !== 'aws')
        return null;
    const backends = doc.architecture.projects.backend || [];
    const dbType = doc.data.primaryDatabase.type;
...
// ─── Terraform Generator ───
function generateTerraform(doc) {
    const cloud = doc.deployment.cloud;
    const generators = {
...
function generateTerraformVariables(doc) {
    const lines = [];
    lines.push('# Generated from SDL');
    lines.push(`# Solution: ${doc.solution.name}`);
    lines.push('');
...
function generateTerraformAws(doc) {
    const lines = [];
    const backends = doc.architecture.projects.backend || [];
    const frontends = doc.architecture.projects.frontend || [];
    const dbType = doc.data.primaryDatabase.type;
...
function generateTerraformGcp(doc) {
    const lines = [];
    const backends = doc.architecture.projects.backend || [];
    const dbType = doc.data.primaryDatabase.type;
    lines.push('# Generated from SDL');
...
function generateTerraformAzure(doc) {
    const lines = [];
    const backends = doc.architecture.projects.backend || [];
    const dbType = doc.data.primaryDatabase.type;
    lines.push('# Generated from SDL');
...
function getDatabaseService(dbType) {
    if (dbType === 'postgres') {
        return [
            '      postgres:',
            '        image: postgres:16',
