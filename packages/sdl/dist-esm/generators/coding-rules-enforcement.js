...
// ─── ESLint Config ───
function buildEslintConfig(doc, backends, frontends) {
...
// ─── Python Lint Config ───
function buildPythonLintConfig(doc) {
...
// ─── Go Lint Config ───
function buildGoLintConfig(doc) {
    const lines = [];
    lines.push('# Auto-generated from SDL — coding rules enforcement');
    lines.push('run:');
...
function buildDependencyCruiserConfig(doc) {
    const services = doc.architecture.services || [];
    const backends = doc.architecture.projects.backend || [];
    const lines = [];
...
// ─── Lint-Staged Config ───
function buildLintStagedConfig(doc, langs) {
    const config = {};
    if (langs.hasTs) {
...
function buildArchitectureTests(doc) {
    const services = doc.architecture.services || [];
...
// ─── Java ArchUnit Tests ───
function buildJavaArchTests(doc) {
    const services = doc.architecture.services || [];
    const lines = [];
    lines.push('// Auto-generated from SDL — architecture enforcement tests');
...
// ─── .NET ArchUnit Tests ───
function buildDotnetArchTests(doc) {
    const lines = [];
    lines.push('// Auto-generated from SDL — architecture enforcement tests');
 ...
 */
function ciVersion(doc, framework) {
...
function buildCiEnforcementWorkflow(doc, langs) {
    const coverage = doc.testing?.coverage?.target || 80;
