 ...
 */
export function generateDockerCompose(doc) {
    const lines = [];
...
function getBackendPort(framework) {
    switch (framework) {
...
function getDatabaseUrl(dbType, doc) {
...
function appendAuthEnv(lines, doc) {
    if (!doc.auth)
        return;
...
function appendDatabaseService(lines, doc) {
    const dbType = doc.data.primaryDatabase.type;
    const name = slugify(doc.solution.name);
    if (dbType === 'postgres') {
        lines.push('');
...
function appendSecondaryDatabase(lines, dbType, role) {
...
function appendQueueService(lines, doc) {
...
function appendSearchService(lines, doc) {
...
function getVolumes(doc) {
    ...
    const dbType = doc.data.primaryDatabase.type;
    if (dbType === 'postgres')
        vols.push('postgres_data');
    else if (dbType === 'mysql')
        vols.push('mysql_data');
    else if (dbType === 'mongodb')
        vols.push('mongo_data');
    if (doc.data.secondaryDatabases) {
        for (const sdb of doc.data.secondaryDatabases) {
            const suffix = slugify(sdb.role || 'secondary');
            if (sdb.type === 'postgres')
                vols.push(`postgres_${suffix}_data`);
            else if (sdb.type === 'mysql')
                vols.push(`mysql_${suffix}_data`);
            else if (sdb.type === 'mongodb')
                vols.push(`mongo_${suffix}_data`);
        }
    }
    if (doc.data.cache?.type === 'redis')
        vols.push('redis_data');
    if (doc.data.queues?.provider === 'rabbitmq')
        vols.push('rabbitmq_data');
    if (doc.data.queues?.provider === 'kafka')
        vols.push('kafka_data');
    if (doc.data.search?.provider === 'elasticsearch')
        vols.push('elasticsearch_data');
    if (doc.data.search?.provider === 'meilisearch')
        vols.push('meilisearch_data');
    if (doc.data.search?.provider === 'typesense')
        vols.push('typesense_data');
    return vols;
}
function generateComposeEnv(doc) {
    const lines = [];
