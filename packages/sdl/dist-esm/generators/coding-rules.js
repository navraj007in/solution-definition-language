 ...
 */
export function generateCodingRules(doc, options) {
    const files = [];
    const targets = options?.targets ?? ['claude', 'cursor', 'copilot', 'aider'];
    ...
    const allProjects = [
        ...(doc.architecture.projects.backend || []).map(p => ({ project: p, type: 'backend' })),
        ...(doc.architecture.projects.frontend || []).map(p => ({ project: p, type: 'frontend' })),
        ...(doc.architecture.projects.mobile || []).map(p => ({ project: p, type: 'frontend' })),
    ];
    for (const { project, type } of allProjects) {
        const componentPath = project.path ?? project.name;
        if (componentPath === '.' || componentPath === './')
            continue; // root component uses root files
    ...
    if (targets.includes('claude'))
        activeTargetLabels.push('claude-code');
    if (targets.includes('cursor'))
        activeTargetLabels.push('cursor');
    if (targets.includes('copilot'))
        activeTargetLabels.push('github-copilot');
    if (targets.includes('aider'))
        activeTargetLabels.push('aider');
    return {
...
function renderCursorRules(doc, content) {
    const header = [
        '---',
...
function buildArchitectureRules(doc) {
...
function buildFileStructureRules(doc) {
    const rules = [];
...
function buildDataAccessRules(doc) {
    const rules = [];
...
function buildApiPatternRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    for (const be of backends) {
...
function buildErrorHandlingRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
...
function buildIntegrationRules(doc) {
    const rules = [];
...
function buildTestingRules(doc) {
    const rules = [];
...
function buildObservabilityRules(doc) {
    const rules = [];
...
function buildSecurityRules(doc) {
    const rules = [];
...
function buildCachingRules(doc) {
    const rules = [];
...
// ─── Code Quality Rules ───
function buildCodeQualityRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    const frontends = doc.architecture.projects.frontend || [];
...
function buildDesignPatternRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    const frontends = doc.architecture.projects.frontend || [];
    const style = doc.architecture.style;
...
function buildFileSizeStructureRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    const frontends = doc.architecture.projects.frontend || [];
...
function buildApiDesignQualityRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    for (const be of backends) {
        const apiStyle = be.apiStyle || 'rest';
...
function buildDatabaseQueryRules(doc) {
    const rules = [];
    const db = doc.data.primaryDatabase;
...
function buildTestingQualityRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
...
function buildPerformanceRules(doc) {
    const rules = [];
    const frontends = doc.architecture.projects.frontend || [];
    const backends = doc.architecture.projects.backend || [];
    // Backend performance
...
function buildImportOrganizationRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    const frontends = doc.architecture.projects.frontend || [];
...
function buildTechDebtAvoidanceRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    const frontends = doc.architecture.projects.frontend || [];
    // Dependency hygiene
...
function buildAccessibilityRules(doc) {
    const rules = [];
    const frontends = doc.architecture.projects.frontend || [];
    const compliance = doc.nonFunctional.compliance?.frameworks || [];
...
function buildInternationalizationRules(doc) {
    const rules = [];
    const frontends = doc.architecture.projects.frontend || [];
    const backends = doc.architecture.projects.backend || [];
...
function buildMigrationDeploymentSafetyRules(doc) {
    const rules = [];
    const db = doc.data.primaryDatabase;
...
function buildDocumentationRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    const frontends = doc.architecture.projects.frontend || [];
    // When to document
...
function buildGitWorkflowRules(doc) {
    const rules = [];
...
function buildResilienceRules(doc) {
    const rules = [];
    const style = doc.architecture.style;
    const backends = doc.architecture.projects.backend || [];
...
function buildInputValidationRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    const frontends = doc.architecture.projects.frontend || [];
    // Defense in depth
...
function buildStateManagementRules(doc) {
    const rules = [];
    const frontends = doc.architecture.projects.frontend || [];
...
function buildConcurrencyRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    const style = doc.architecture.style;
    const db = doc.data.primaryDatabase;
...
function buildConfigurationRules(doc) {
    const rules = [];
    const backends = doc.architecture.projects.backend || [];
    // Config hierarchy
    rules.push('');
...
// ─── Markdown Renderer ───
function renderRulesMarkdown(doc, rules, adrRules = [], opts = {}) {
    const lines = [];
    ...
    for (const fe of frontends) {
        ...
        const extras = [];
        if (fe.rendering)
            extras.push(fe.rendering.toUpperCase());
        if (fe.styling)
            extras.push(fe.styling);
        if (fe.stateManagement && fe.stateManagement !== 'none')
            extras.push(fe.stateManagement);
...
// ─── Per-Project Rules ───
function renderProjectRules(doc, project, type) {
    const lines = [];
    const componentPath = project.path ?? project.name;
...
function getBackendFileRules(be) {
    const rules = [];
    const fw = be.framework;
    if (fw === 'nodejs') {
...
function getFrontendFileRules(fe) {
    const rules = [];
...
// ─── Forbidden Patterns ───
function getForbiddenPatterns(doc) {
    const patterns = [];
