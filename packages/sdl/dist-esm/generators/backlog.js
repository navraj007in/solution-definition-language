 ...
 */
export function generateBacklog(doc) {
    const epics = [];
    let storyCounter = 1;
    // 1. Setup epic (always)
...
// ─── Epic Builders ───
function buildSetupEpic(doc, startId) {
...
function buildAuthEpic(doc, startId) {
    const auth = doc.auth;
...
function buildPersonaEpic(doc, persona, startId) {
    const stories = [];
    let id = startId;
...
function buildInfraEpic(doc, startId) {
    const stories = [];
    let id = startId;
    // CI/CD
    stories.push({
...
// ─── Markdown Renderer ───
function renderBacklogMarkdown(doc, epics, totalStories) {
    const lines = [];
