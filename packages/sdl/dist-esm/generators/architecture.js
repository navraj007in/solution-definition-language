...
export function generateArchitectureDiagram(doc) {
    ...
    for (const fe of frontends)
        lines.push(`    class ${sanitize(fe.name)} frontend`);
    for (const mob of mobiles)
        lines.push(`    class ${sanitize(mob.name)} frontend`);
    for (const be of backends)
        lines.push(`    class ${sanitize(be.name)} backend`);
    if (doc.auth?.provider)
        lines.push('    class auth_provider auth');
    lines.push('    class primary_db data');
    if (doc.data.cache && doc.data.cache.type !== 'none')
        lines.push('    class cache data');
