...
// ─── Resolver ───
function hasAnyPackage(deps, packages) {
    return packages.some((pkg) => pkg in deps);
}
function hasAnyFile(files, patterns) {
    return patterns.some((pattern) => files.some((f) => f === pattern || f.endsWith(`/${pattern}`) || f.includes(pattern)));
}
function hasDeliverable(deliverables, types) {
    return types.some((t) => deliverables.some((d) => d.type === t));
}
function hasPhase(completedPhases, phases) {
...
function resolveBackendServices(doc, ev) {
    const services = doc.architecture.services ?? [];
    if (services.length === 0 && (doc.architecture.projects.backend ?? []).length === 0)
        return [];
    const files = ev.codebaseSignals?.detectedFiles ?? [];
    ...
    for (const svc of services) {
        ...
        const svcPath = svc.path;
        if (svcPath && svcPath !== '.')
            pathPatterns.push(svcPath);
...
function resolveFrontends(doc, ev) {
    ...
    return frontends.map((fe) => {
        ...
        const fePath = fe.path;
        if (fePath && fePath !== '.')
            pathPatterns.push(fePath);
...
function resolveDatabases(doc, ev) {
    const dbs = [];
    const deps = ev.codebaseSignals?.detectedDependencies ?? {};
    const files = ev.codebaseSignals?.detectedFiles ?? [];
...
function resolveIntegrations(doc, ev) {
    if (!doc.integrations)
        return [];
    const deps = ev.codebaseSignals?.detectedDependencies ?? {};
    const files = ev.codebaseSignals?.detectedFiles ?? [];
...
function resolveInfrastructure(doc, ev) {
    const files = ev.codebaseSignals?.detectedFiles ?? [];
...
function resolveArchitectureDecisions(_doc, ev) {
...
function resolveObservability(doc, ev) {
...
// ─── Main Resolver ───
function buildCategorySummary(category, components) {
    if (components.length === 0) {
        return {
            category,
...
export function resolveProgress(doc, evidence, overrides = [], projectId = '') {
