 ...
 */
export function getPlatformPricing(platformId: string): PlatformPricing | undefined {
 ...
 */
export function getComputeCost(platformId: string, scale: 'small' | 'medium' | 'large'): number {
  const platform = getPlatformPricing(platformId);
  if (!platform) return 15;
 ...
 */
export function getDatabaseCost(platformId: string, scale: 'small' | 'medium' | 'large'): number {
  const platform = getPlatformPricing(platformId);
  if (!platform) return 15;
