...
import type { AdrImpactSuggestion } from '../adr-impact';
...
function loadCompiled(name: string): SDLDocument {
...
describe('diff()', () => {
  ...
  it('detects a changed scalar field', () => {
    ...
    const b = JSON.parse(JSON.stringify(a)) as SDLDocument;
...
describe('generateAdrDraftFromDiff()', () => {
  ...
  it('draft alternatives is an array', () => {
    ...
    const doc = loadCompiled('taskflow.yaml');
    const draft = generateAdrDraftFromDiff(doc as unknown as Record<string, unknown>, impact.suggestions[0]);
...
`.trim();
    ...
    assert.equal((result.document as any).solution.stage, 'MVP');
...
`.trim();
    ...
    assert.equal((result.document as any).auth.strategy, 'oidc');
    assert.equal((result.document as any).auth.sessions.accessToken, 'jwt');
    assert.ok(result.compilesClean, `errors: ${result.remainingErrors.join(', ')}`);
  });
...
`.trim();
    ...
    assert.ok(Array.isArray((result.document as any).contracts.apis));
    assert.ok(result.compilesClean, `errors: ${result.remainingErrors.join(', ')}`);
  });
...
`.trim();
    ...
    assert.ok(Array.isArray((result.document as any).slos.services));
    assert.ok(result.compilesClean, `errors: ${result.remainingErrors.join(', ')}`);
  });
...
describe('generateComplianceChecklist()', () => {
  ...
  it('produces framework content for GDPR', () => {
    const doc = loadCompiled('taskflow.yaml');
    (doc as any).compliance = { frameworks: [{ name: 'GDPR', applicable: true }] };
    const result = generateComplianceChecklist(doc);
    const content = result.files[0].content;
  ...
  it('metadata includes framework names', () => {
    const doc = loadCompiled('taskflow.yaml');
    (doc as any).compliance = { frameworks: [{ name: 'SOC2-Type2', applicable: true }] };
    const result = generateComplianceChecklist(doc);
    assert.ok((result.metadata.frameworks as string[]).includes('SOC2-Type2'));
  });
});
...
describe('summarizeGenerationResults()', () => {
  it('returns a string with total count', () => {
    const doc = loadCompiled('taskflow.yaml');
    const results = ['openapi', 'adr'].map((t) => generate(doc, t as any)).filter(Boolean) as any[];
