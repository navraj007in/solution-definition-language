...
describe('validate', () => {
  describe('valid input', () => {
    ...
    it('allows x- extension fields', () => {
      const data = fixture('taskflow.yaml') as Record<string, unknown>;
  ...
  describe('conditional rules (allOf)', () => {
    it('rejects Azure + CloudFormation', () => {
      ...
      const codes = result.errors.map(e => e.code);
      assert.ok(
        codes.includes('INCOMPATIBLE_CLOUD_IAC'),
        `Expected INCOMPATIBLE_CLOUD_IAC in ${JSON.stringify(codes)}`
      );
    });
    ...
    it('rejects MongoDB + EF Core', () => {
      ...
      const codes = result.errors.map(e => e.code);
      assert.ok(
        codes.includes('INCOMPATIBLE_DATABASE_ORM'),
        `Expected INCOMPATIBLE_DATABASE_ORM in ${JSON.stringify(codes)}`
      );
    });
    ...
    it('rejects PII without encryptionAtRest', () => {
      ...
      const codes = result.errors.map(e => e.code);
      assert.ok(
        codes.includes('PII_REQUIRES_ENCRYPTION'),
        `Expected PII_REQUIRES_ENCRYPTION in ${JSON.stringify(codes)}`
      );
    });
  });
  ...
  describe('enum validation', () => {
    it('rejects invalid architecture style', () => {
      const data = fixture('taskflow.yaml') as Record<string, unknown>;
      (data as any).architecture.style = 'monolith';
      const result = validate(data);
      assert.equal(result.valid, false);
  ...
  describe('additionalProperties', () => {
    it('rejects unknown fields without x- prefix', () => {
      const data = fixture('taskflow.yaml') as Record<string, unknown>;
      (data as any).solution.unknownField = 'bad';
      const result = validate(data);
      assert.equal(result.valid, false);
