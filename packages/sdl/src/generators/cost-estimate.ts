...
function estimateCosts(doc: SDLDocument): CostItem[] {
...
function estimateCompute(cloud: string, name: string, framework: string, scale: Scale): CostItem {
  const costs: Record<string, { tier: string; cost: number; notes: string }> = {
...
function estimateFrontendHosting(cloud: string, name: string, framework: string, scale: Scale): CostItem {
...
function estimateDatabase(dbType: string, hosting: string, cloud: string, scale: Scale): CostItem {
  ...
  const managedCosts: Record<string, Record<string, number>> = {
...
function estimateCache(cacheType: string, cloud: string, scale: Scale): CostItem {
  const costs: Record<string, number> = { small: 0, medium: 15, large: 50 };
...
function estimateAuth(provider: string, scale: Scale): CostItem {
  const costs: Record<string, { free: number; cost: number; notes: string }> = {
...
function estimatePayments(provider: string): CostItem {
  return {
    category: 'Payments',
    service: displayName(provider),
...
function estimateEmail(provider: string, scale: Scale): CostItem {
  const costs: Record<string, { cost: number; notes: string }> = {
...
function estimateMonitoring(provider: string, scale: Scale): CostItem {
  const costs: Record<string, { cost: number; notes: string }> = {
...
function estimateCdn(provider: string, scale: Scale): CostItem {
  return {
    category: 'CDN',
    service: displayName(provider),
...
function estimateCiCd(provider: string): CostItem {
  const costs: Record<string, { cost: number; notes: string }> = {
...
function renderCostMarkdown(doc: SDLDocument, items: CostItem[], totalMonthly: number): string {
  const lines: string[] = [];
  const scale = getScale(doc);
...
function getScale(doc: SDLDocument): Scale {
...
function scaleDescription(doc: SDLDocument): string {
  const m1 = doc.nonFunctional.scaling.expectedUsersMonth1;
  const y1 = doc.nonFunctional.scaling.expectedUsersYear1;
  const parts: string[] = [];
  if (m1) parts.push(`${m1} users month 1`);
  if (y1) parts.push(`${y1} users year 1`);
