...
export interface ParsedADR {
  /** e.g. "ADR-003" or "ADR-003: Database Selection" */
  id: string;
  title: string;
  status: 'accepted' | 'superseded' | 'deprecated' | 'proposed' | 'unknown';
  decision: string;
  consequences: string;
  /** The rejected alternatives, used to produce "do not suggest X" rules */
 ...
 */
export function parseADR(markdown: string): ParsedADR | null {
  const lines = markdown.split('\n');

  let title = '';
  let status: ParsedADR['status'] = 'unknown';
  let decision = '';
  let consequences = '';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentSection === 'status' && line && !status.match(/^(accepted|superseded|deprecated|proposed)$/)) {
      ...
      if (s.includes('accepted')) status = 'accepted';
      else if (s.includes('superseded')) status = 'superseded';
 ...
 */
export function parseADRs(markdownFiles: string[]): ParsedADR[] {
  return markdownFiles
    .map(parseADR)
    .filter((adr): adr is ParsedADR => adr !== null && adr.status === 'accepted');
}
 ...
 */
export function adrsToCodingRules(adrs: ParsedADR[]): string[] {
  const rules: string[] = [];
  ...
  for (const adr of adrs) {
    ...
    if (adr.rejectedAlternatives.length > 1) {
      ...
      if (rejected.length > 0) {
        rules.push(
          `${adr.id} (rejected alternatives — do not suggest): ${rejected.join(', ')}`,
        );
      }
    }
  }
