...
function renderGuide(doc: SDLDocument): string {
  const lines: string[] = [];
  ...
  if (doc.auth) lines.push('- [ ] Authentication flow works');
  lines.push('- [ ] Database connections are healthy');
  if (doc.integrations?.payments) lines.push('- [ ] Payment webhook is reachable');
  if (doc.deployment.networking?.customDomain) lines.push('- [ ] Custom domain resolves correctly');
...
function getRequiredTools(doc: SDLDocument): string[] {
  ...
  const cloud = doc.deployment.cloud;
  if (cloud === 'aws') tools.push('AWS CLI v2');
  if (cloud === 'gcp') tools.push('gcloud CLI');
  if (cloud === 'azure') tools.push('Azure CLI');
  if (cloud === 'vercel') tools.push('Vercel CLI (`npm i -g vercel`)');
  if (cloud === 'railway') tools.push('Railway CLI (`npm i -g @railway/cli`)');
  if (cloud === 'fly-io') tools.push('Fly CLI (`flyctl`)');

  if (doc.deployment.infrastructure?.iac === 'terraform') tools.push('Terraform 1.5+');
  if (doc.deployment.infrastructure?.iac === 'pulumi') tools.push('Pulumi CLI');

  const ormBackend = backends.find((be) => be.orm);
  if (ormBackend?.orm === 'prisma') tools.push('Prisma CLI (`npx prisma`)');

  return [...new Set(tools)];
}

function getEnvVars(doc: SDLDocument): { name: string; example: string; description: string }[] {
  const vars: { name: string; example: string; description: string }[] = [];
...
function getDatabaseUrlExample(dbType: string): string {
  switch (dbType) {
...
function getManagedDbInstructions(dbType: string, cloud: string): string {
...
function getCreateDbCommand(dbType: string): string {
  switch (dbType) {
...
function getMigrationCommands(orm: string): string {
  switch (orm) {
    case 'prisma':
...
function getBuildCommands(framework: string, type: 'backend' | 'frontend'): string {
  switch (framework) {
    case 'nodejs':
      return 'npm ci\nnpm run build';
    case 'nextjs':
...
function getDeployInstructions(doc: SDLDocument): string {
  const cloud = doc.deployment.cloud;
  const lines: string[] = [];
...
function getRollbackInstructions(cloud: string): string {
  const lines: string[] = [];
...
function countProjects(doc: SDLDocument): number {
