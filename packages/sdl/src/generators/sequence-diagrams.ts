...
function buildAuthFlow(doc: SDLDocument): { name: string; slug: string; content: string } {
  const auth = doc.auth!;
...
function buildCoreFlow(
  doc: SDLDocument,
  flow: { name: string; steps?: string[]; priority?: string },
...
function buildGoalFlow(
  doc: SDLDocument,
  personaName: string,
  goal: string,
): { name: string; slug: string; content: string } | null {
  const frontends = doc.architecture.projects.frontend || [];
  const backends = doc.architecture.projects.backend || [];
  const clientLabel = frontends.length > 0 ? `${frontends[0].name} (Frontend)` : 'Client';
  const apiLabel = backends.length > 0 ? `${backends[0].name} (API)` : 'API';
...
function classifyStep(step: string): { type: 'user-input' | 'api-call' | 'display' | 'generic' } {
  const lower = step.toLowerCase();
  if (/^(enter|fill|type|input|select|choose|click|submit|upload)/.test(lower)) return { type: 'user-input' };
  if (/^(send|post|get|fetch|save|create|update|delete|call|request)/.test(lower)) return { type: 'api-call' };
  if (/^(display|show|render|redirect|navigate|view|see)/.test(lower)) return { type: 'display' };
  return { type: 'generic' };
}
