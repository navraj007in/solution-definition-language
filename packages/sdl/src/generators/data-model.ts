...
function inferEntities(doc: SDLDocument): Entity[] {
  const entities: Entity[] = [];
  const seen = new Set<string>();
...
function inferRelations(entities: Entity[], doc: SDLDocument): Relation[] {
...
function renderOrmSchema(orm: string, entities: Entity[], relations: Relation[], doc: SDLDocument): string | null {
  switch (orm) {
    case 'prisma':
...
function renderPrismaSchema(entities: Entity[], relations: Relation[], doc: SDLDocument): string {
  ...
  for (const entity of entities) {
    ...
    for (const field of entity.fields) {
      ...
      if (field.pk) modifiers.push('@id @default(uuid())');
      if (field.unique) modifiers.push('@unique');
      if (field.name === 'created_at') modifiers.push('@default(now()) @map("created_at")');
      if (field.name === 'updated_at') modifiers.push('@updatedAt @map("updated_at")');
      if (field.comment?.startsWith('FK')) continue; // FK field handled by relation
...
function renderTypeOrmSchema(entities: Entity[], relations: Relation[]): string {
  const lines: string[] = [];
  lines.push('// Generated from SDL');
...
function renderSequelizeSchema(entities: Entity[], relations: Relation[]): string {
  const lines: string[] = [];
  lines.push('// Generated from SDL');
  ...
  for (const entity of entities) {
    ...
    for (const field of entity.fields) {
      if (field.pk) {
        ...
        const opts: string[] = [`type: ${seqType}`];
        if (!field.required) opts.push('allowNull: true');
...
function renderMongooseSchema(entities: Entity[], relations: Relation[]): string {
  const lines: string[] = [];
  lines.push('// Generated from SDL');
  ...
  for (const entity of entities) {
    ...
    for (const field of entity.fields) {
      if (field.pk) continue; // Mongoose uses _id
    ...
    for (const field of entity.fields) {
      ...
      if (field.comment?.startsWith('FK')) {
        ...
        const opts: string[] = [`type: ${mongoType}`];
...
function renderSqlAlchemySchema(entities: Entity[], relations: Relation[]): string {
  const lines: string[] = [];
  lines.push('# Generated from SDL');
  ...
  for (const entity of entities) {
    ...
    for (const field of entity.fields) {
      ...
      const opts: string[] = [];
      if (field.pk) opts.push('primary_key=True', 'default=uuid.uuid4');
      if (field.unique) opts.push('unique=True');
      if (!field.required && !field.pk) opts.push('nullable=True');
      if (field.name === 'created_at') opts.push('default=datetime.utcnow');
      if (field.name === 'updated_at') opts.push('default=datetime.utcnow', 'onupdate=datetime.utcnow');
...
function renderEfCoreSchema(entities: Entity[], relations: Relation[]): string {
  const lines: string[] = [];
  lines.push('// Generated from SDL');
  ...
  for (const entity of entities) {
    ...
    for (const field of entity.fields) {
      if (field.comment?.startsWith('FK')) continue;
      const csType = toCSharpType(field.type);
      const nullable = !field.required && !field.pk ? '?' : '';
      if (field.pk) lines.push('    [Key]');
      if (field.unique) lines.push(`    [MaxLength(256)]`);
...
function renderGormSchema(entities: Entity[], relations: Relation[]): string {
  const lines: string[] = [];
  lines.push('// Generated from SDL');
  ...
  for (const entity of entities) {
    ...
    for (const field of entity.fields) {
      if (field.pk) {
        ...
        const tags: string[] = [];
        if (!field.required) tags.push('default:null');
        if (field.unique) tags.push('uniqueIndex');
        if (field.required) tags.push('not null');
...
function toMongooseType(type: string): string {
  const map: Record<string, string> = {
    uuid: 'String',
    varchar: 'String',
    text: 'String',
...
function toSqlAlchemyType(type: string): string {
  const map: Record<string, string> = {
...
function toGoType(type: string): string {
  const map: Record<string, string> = {
    uuid: 'uuid.UUID',
    varchar: 'string',
    text: 'string',
...
function ormSchemaFilename(orm: string): string {
  switch (orm) {
...
function toSequelizeType(type: string): string {
  const map: Record<string, string> = {
    uuid: 'DataTypes.UUID',
...
function pluralizeLower(s: string): string {
  ...
  if (lower.endsWith('s') || lower.endsWith('x') || lower.endsWith('z') || lower.endsWith('sh') || lower.endsWith('ch')) return lower + 'es';
  return lower + 's';
}
