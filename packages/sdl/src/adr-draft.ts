...
import type { AdrCategory, AdrImpactSuggestion } from './adr-impact';
...
export interface AdrDraft {
  ...
  sourceCategory: AdrCategory;
  sourceReasonCodes: string[];
}

/** Minimal SDL shape — we only need a few fields for draft generation. */
interface SdlDocLike {
  solution?: { name?: string; stage?: string };
  architecture?: { style?: string; projects?: { frontend?: unknown[]; backend?: unknown[]; mobile?: unknown[] } };
  auth?: { strategy?: string; provider?: string; roles?: string[]; mfa?: boolean; socialProviders?: string[] };
  data?: {
    primaryDatabase?: { type?: string; hosting?: string };
    cache?: { type?: string; useCase?: string[] };
  };
  deployment?: {
    cloud?: string;
    runtime?: { frontend?: string; backend?: string };
    ciCd?: { provider?: string; environments?: Array<{ name: string }> };
  };
}

// ── Category → Draft mapping ──

const CATEGORY_DRAFTS: Record<AdrCategory, (doc: SdlDocLike, suggestion: AdrImpactSuggestion) => AdrDraft> = {
 ...
 */
export function generateAdrDraftFromDiff(
  sdlDoc: Record<string, unknown>,
  suggestion: AdrImpactSuggestion,
): AdrDraft {
  const doc = sdlDoc as unknown as SdlDocLike;
...
function joinReasons(s: AdrImpactSuggestion): string {
