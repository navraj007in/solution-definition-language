...
export type AdrRuleCode =
  ...
  | 'CACHE_STRATEGY_CHANGED'
  | 'CICD_CHANGED'
  | 'COMPONENT_ADDED'
  | 'COMPONENT_REMOVED'
  | 'COMPONENT_RUNTIME_CHANGED'
  | 'COMPONENT_FRAMEWORK_CHANGED'
  | 'COMPONENT_LANGUAGE_CHANGED';

export type Severity = 'low' | 'medium' | 'high';
export type Confidence = 'low' | 'medium' | 'high';

export interface AdrImpactSuggestion {
  category: AdrCategory;
  severity: Severity;
  confidence: Confidence;
  reasons: string[];
  reasonCodes: AdrRuleCode[];
  affectedPaths: string[];
  affectedComponents: string[];
}

export interface AdrImpactResult {
  suggestions: AdrImpactSuggestion[];
  summary: {
    totalSuggestions: number;
    highestSeverity: Severity | null;
    hasHighSignalChanges: boolean;
    compoundTriggers: CompoundTrigger[];
  };
}

// ── Input types (mirrors desktop SdlDiffResult) ──

interface SdlComponentDiff {
  name: string;
  changes: string[];
}

interface SdlSectionChange {
  section: string;
  fields: Array<{
    path: string;
    before: string | null;
...
interface SdlDiffResult {
  added: Array<{ name: string; type: string }>;
  removed: Array<{ name: string; type: string }>;
  modified: SdlComponentDiff[];
  crossCuttingChanged: boolean;
  sectionChanges: SdlSectionChange[];
  hasChanges: boolean;
}
...
function hasSignificantChange(changes: string[]): boolean {
...
function classifySectionChanges(sectionChanges: SdlSectionChange[]): RawHit[] {
  const hits: RawHit[] = [];
  ...
  for (const sc of sectionChanges) {
    ...
    // Auth section
    if (section === 'auth') {
      const significantFields = sc.fields.filter(
        (f) => !f.path.startsWith('x-'),
      );
    ...
    if (section === 'data') {
      ...
      const dbFields = sc.fields.filter(
        (f) => f.path.startsWith('primaryDatabase') || f.path.startsWith('primary_database'),
      );
      ...
      // Cache changes
      const cacheFields = sc.fields.filter(
        (f) => f.path.startsWith('cache'),
      );
    ...
    if (section === 'deployment') {
      const cloudFields = sc.fields.filter(
        (f) => f.path === 'cloud' || f.path.startsWith('cloud.') || f.path.startsWith('runtime'),
      );
      ...
      const ciCdFields = sc.fields.filter(
        (f) => f.path.startsWith('ciCd') || f.path.startsWith('ci_cd'),
      );
...
function classifyComponentChanges(diff: SdlDiffResult): RawHit[] {
  const hits: RawHit[] = [];
  ...
  for (const comp of diff.modified) {
    if (!hasSignificantChange(comp.changes)) continue;
...
function classifyArchitectureStyle(sectionChanges: SdlSectionChange[]): RawHit[] {
  const hits: RawHit[] = [];

  for (const sc of sectionChanges) {
    if (sc.section === 'solution') {
      // architecture.style is often under solution-level changes
      const styleField = sc.fields.find(
        (f) => f.path === 'architecture.style' || f.path === 'style',
      );
      if (styleField) {
        hits.push({
          category: 'architecture-style',
...
function aggregate(hits: RawHit[]): AdrImpactSuggestion[] {
  const byCategory = new Map<AdrCategory, RawHit[]>();
  ...
  const suggestions: AdrImpactSuggestion[] = [];
  ...
  for (const [category, categoryHits] of byCategory) {
    ...
    const affectedComponents = [
      ...new Set(categoryHits.map((h) => h.component).filter(Boolean) as string[]),
    ];
...
function pickHighest(levels: Array<Severity | Confidence>): Severity & Confidence {
...
export type CompoundTrigger = 'PLATFORM_MIGRATION' | 'TECH_STACK_OVERHAUL' | 'DATA_LAYER_REDESIGN';

interface CompoundRule {
  trigger: CompoundTrigger;
  /** At least N of these categories must be present to fire. */
  requiredCategories: AdrCategory[];
  minMatch: number;
  label: string;
  boostSeverity: Severity;
}
...
function applyCompoundTriggers(suggestions: AdrImpactSuggestion[]): CompoundTrigger[] {
  const presentCategories = new Set(suggestions.map((s) => s.category));
  const fired: CompoundTrigger[] = [];
 ...
 */
export function classifyDiffForAdr(diff: SdlDiffResult): AdrImpactResult {
  if (!diff.hasChanges) {
    return {
      suggestions: [],
  ...
  const confidenceOrd: Record<Confidence, number> = { high: 3, medium: 2, low: 1 };
  suggestions.sort((a, b) => {
    const sDiff = severityOrder[b.severity] - severityOrder[a.severity];
    if (sDiff !== 0) return sDiff;
  ...
  const hasHighSignalChanges = suggestions.some(
    (s) => s.severity === 'high' && s.confidence === 'high',
  );
