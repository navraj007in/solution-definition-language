...
import type { ComponentSpec, ArtifactCheck } from './verification-spec.types';
import type { ProgressCategory } from './progress.types';

function componentRoot(component: { name?: string; path?: string }): string {
  if ((component as any).path && (component as any).path !== '') return (component as any).path;
  return (component.name ?? 'unknown').toLowerCase().replace(/\s+/g, '-');
}

// Detect manifest file based on runtime/language
function manifestForRuntime(root: string, runtime?: string, language?: string): { path: string; type: string } {
...
const DB_PACKAGES: Record<string, Record<string, string[]>> = {
  npm: {
...
const AUTH_PACKAGES: Record<string, Record<string, string[]>> = {
  npm: {
...
function pkgManifestPath(root: string): string {
  return root === '.' || root === './' ? 'package.json' : `${root}/package.json`;
}

export function deriveVerificationSpec(doc: SDLDocument): ComponentSpec[] {
  const specs: ComponentSpec[] = [];

  // Backend services
  const backendProjects = doc.architecture?.projects?.backend ?? [];
  const services = (doc.architecture as any)?.services ?? [];
  ...
  for (const svc of allBackend) {
    ...
    specs.push({
      id: `service:${svc.name}`,
      category: 'backend_services' as ProgressCategory,
      label: svc.name,
      componentRoot: root,
  ...
  for (const fe of frontendProjects) {
    ...
    const feType = ((fe as any).type ?? '').toString().toLowerCase();
    const checks: ArtifactCheck[] = [];
  ...
  if (data?.primaryDatabase) {
    ...
    const dbType = (db.type ?? (db as any).engine ?? 'unknown').toString().toLowerCase();
    ...
    const ownerLang = ownerBackend ? ((ownerBackend as any).language ?? '').toString() : '';
    ...
    if (pkgs.length > 0) {
      checks.push(...pkgs.map(pkg => ({ kind: 'package_installed' as const, pkg, manifestPath: manifest })));
    }
    // ORM schema — check in backend root
    const orm = ownerBackend ? ((ownerBackend as any).orm ?? '').toString().toLowerCase() : '';
    ...
    if (checks.length > 0) {
      specs.push({
        ...
        category: 'databases' as ProgressCategory,
        label: `${db.name ?? dbType} (${(db as any).hosting ?? 'managed'})`,
  ...
  // Cache
  if (data?.cache && (data.cache as any).type && (data.cache as any).type !== 'none') {
    const cacheType = (data.cache as any).type.toString().toLowerCase();
    const ownerRoot = allBackend[0] ? componentRoot(allBackend[0]) : '.';
    const manifest = pkgManifestPath(ownerRoot);
    const CACHE_PACKAGES: Record<string, string[]> = {
    ...
    const checks: ArtifactCheck[] = pkgs.map(pkg => ({ kind: 'package_installed' as const, pkg, manifestPath: manifest }));
  ...
  if (auth) {
    ...
    const ownerLang = ownerBackendAuth ? ((ownerBackendAuth as any).language ?? '').toString() : '';
    const mf = manifestForRuntime(ownerRoot, ownerRt, ownerLang);
    const manifest = mf.type === 'npm' ? pkgManifestPath(ownerRoot) : mf.path;
    ...
    if (strategy) providerTypes.push(strategy);
    const providers = auth.providers;
    if (providers && typeof providers === 'object') {
      for (const p of Object.values(providers)) {
        const pType = ((p as any)?.type ?? '').toString().toLowerCase();
        if (pType) providerTypes.push(pType);
      }
    }
    const singleProvider = (auth.provider ?? '').toString().toLowerCase();
    if (singleProvider) providerTypes.push(singleProvider);
    ...
    for (const pType of providerTypes) {
      ...
      if (pkgs) pkgs.forEach(p => allPkgs.add(p));
    }
    ...
    const checks: ArtifactCheck[] = [...allPkgs].map(pkg => ({
      kind: 'package_installed' as const, pkg, manifestPath: manifest,
    }));
    ...
    specs.push({
      ...
      category: 'auth' as ProgressCategory,
      label,
      componentRoot: ownerRoot,
      requiredChecks: checks,
    });
  ...
  // Infrastructure
  const deployment = (doc as any).deployment;
  if (deployment) {
    const checks: ArtifactCheck[] = [
      { kind: 'file_exists', path: 'docker-compose.yml' },
    ];
    ...
    specs.push({
      ...
      category: 'infrastructure' as ProgressCategory,
  ...
  const observability = (doc as any).observability;
  if (observability) {
    ...
    const ownerLang = ownerBackendObs ? ((ownerBackendObs as any).language ?? '').toString() : '';
    const mf = manifestForRuntime(ownerRoot, ownerRt, ownerLang);
    const manifest = mf.type === 'npm' ? pkgManifestPath(ownerRoot) : mf.path;

    const LOGGING_PACKAGES: Record<string, Record<string, string[]>> = {
    ...
    const checks: ArtifactCheck[] = logPkgs.map(pkg => ({
      kind: 'package_installed' as const, pkg, manifestPath: manifest,
    }));

    specs.push({
      id: 'observability:logging',
      category: 'observability' as ProgressCategory,
