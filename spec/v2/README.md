...
**Status: active specification for the 2.x line**, published as baseline `sdl-v2.0`. [SDL v1.1](../SDL-v1.1.md) remains active for the 1.x line. This directory is the v2 language contract. `@sdl/core` still implements v1.1 only, so `sdlVersion: "2.0"` is not yet accepted by current tooling; that is a package support gap, not a specification limit. [`packages/core-v2/`](../../packages/core-v2/README.md) is the early-stage v2 package implementation — the input profile (IN-001–IN-005), part of identity/references (ID-001–ID-004), import resolution/composition (IM-001–IM-006), and full-document structural validation against `sdl-v2.schema.json` are done so far, checked against this directory's `conformance/cases.yaml` and `conformance/full-document.yaml`.
