---
name: sdl-author
description: Generate, repair, or review Solution Definition Language (SDL) v1.1 YAML from product requirements, architecture notes, repo discovery summaries, diagrams, or user descriptions. Use when Codex needs to create a valid `solution.sdl.yaml`, modular `.sdl.yaml` files, SDL fragments, schema-compliant fixes, assumptions, or review items for SDL authoring.
---
...
Use this skill to turn architecture intent into valid SDL v1.1. Favor small, schema-valid SDL over comprehensive-looking YAML that invents facts.

## Reference Loading

1. If working in the `solution-definition-language` repo, load `reference/ai-authoring.md` first. It is the compact machine reference.
...
5. Validate and repair.
   - Run `scripts/validate-sdl.mjs <file>` when a file exists.
   - If validation fails, fix the SDL and validate again.
   - Mention remaining warnings or assumptions in the final response.
...
```bash
node scripts/validate-sdl.mjs path/to/solution.sdl.yaml
```

Useful options:

```bash
node scripts/validate-sdl.mjs path/to/solution.sdl.yaml --repo /path/to/solution-definition-language
node scripts/validate-sdl.mjs path/to/solution.sdl.yaml --json
```
